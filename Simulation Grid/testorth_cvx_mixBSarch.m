...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%function [y_cvx_mixA, mu_cvx_mixA, cvx_time_mixA, cvx_status_mixA, frac_ue_mixA, frac_ue_perS_mixA, macro_ue_mixA,  macro_ue_only_mixA, pico_ue_mixA, pico_ue_only_mixA, cluster_ue_mixA, cluster_ue_only_mixA, BS_time_mixA, rate_cvx_mixA, utility_cvx_mixA] = cvx_mixBSarch(nu, nB, narch, nCluster, SU_js, act_u_cluster, actBSv, archv, c_zf_rho)

% =================================================
% some matrixs to be used
% number of clusters per user
nCluster_perU_orth   =   size(act_u_cluster_orth, 2);
% =================================================
% the matrix to get j\in C, |C|=s
actBS_arch_orth              =   zeros(nB*narch_orth, nCluster_orth);
for i = 1 : narch_orth
    actBS_arch_orth(nB*(i-1)+1:nB*i, :) = actBSv_orth'.*repmat(archv_orth(i,:), nB, 1);
end
% =================================================
% the matrix indicating j\in C |C|=s for each user
% col:(j1s1, j2s1, ..., j1s2, j2s2, ...)
% row: c1u1, c2u1, ..., c1u2, c2u2, ...
actBS_arch_u_orth            =   zeros(nB * narch_orth, nCluster_perU_orth * nu);
for i = 1 : nCluster_perU_orth
    actBS_arch_u_orth(:, i:nCluster_perU_orth:nu*nCluster_perU_orth) = actBS_arch_orth(:, act_u_cluster_orth(:, i));
end
% =================================================
% the matrix to get |C|=s
actcluster_arch_u_orth     	=   zeros(narch_orth*nu, nCluster_perU_orth);
for i = 1 : narch_orth
    actcluster_arch_u_orth(nu*(i-1)+1: nu*i, :) = reshape(archv_orth(i, act_u_cluster_orth), nu, nCluster_perU_orth);
end
% =================================================
% matrix to indicator 
% row: j1s1(u1, u2, u3, ...), j2s1(u1,u2,u3,...), ..., j1s2(u1,u2,u3...), j2s2(u1,u2,u3)
% col: c1, c2, c3, ...
act_arch_u_cluster_orth      =   reshape((reshape(actBS_arch_u_orth,[],nu))',[], nCluster_perU_orth);
% =================================================
% start cvx 
size( actBS_arch_u_orth*reshape(ones(nCluster_perU_orth, nu), nCluster_perU_orth*nu, 1)./SU_js_orth)
size(SU_js_orth)
nB*narch_orth
start_time = tic;
cvx_expert true
cvx_clear
cvx_begin
    variable y(nCluster_perU_orth, nu);
    variable mu(nB, narch_orth);
    maximize( sum(log(sum(c_zf_orth.*y, 1))) );
    %maximize( geo_mean(sum(c_zf_rho.*y, 1)) );
subject to
    actBS_arch_u_orth*reshape(y, nCluster_perU_orth*nu, 1)./SU_js_orth <= reshape(mu,nB*narch_orth,1);
    sum(act_arch_u_cluster_orth.*repmat(y',nB*narch_orth,1),2) <= kron(reshape(mu,nB*narch_orth,1), ones(nu,1));
    sum(y, 1) <= 1;
    sum(mu, 2) <= 1;
    y >= 0;
    mu >= 0;
