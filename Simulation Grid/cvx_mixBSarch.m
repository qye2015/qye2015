...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [y_cvx_mixA, mu_cvx_mixA, cvx_time_mixA, cvx_status_mixA, frac_ue_mixA, frac_ue_perS_mixA, macro_ue_mixA,  macro_ue_only_mixA, pico_ue_mixA, pico_ue_only_mixA, cluster_ue_mixA, cluster_ue_only_mixA, BS_time_mixA, rate_cvx_mixA, utility_cvx_mixA] = cvx_mixBSarch(nu, nM, nP, nB, narch, nCluster, SU_js, act_u_cluster, actBSv, archv, c_zf_rho)
...
cvx_begin
    ...
    variable lambda(nB, narch);
    maximize( sum(log(sum(c_zf_rho.*y, 1))) );
subject to
    actBS_arch_u*reshape(y, nCluster_perU*nu, 1)./SU_js <= reshape(lambda, nB*narch,1);
    sum(act_arch_u_cluster.*repmat(y',nB*narch,1),2) <= kron(reshape(lambda, nB*narch,1), ones(nu,1));
    sum(y, 1) <= 1;
    sum(lambda, 2) <= 1;
    y >= 0;
    lambda >= 0;
